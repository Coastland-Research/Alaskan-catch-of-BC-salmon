## Stikine Sockeye escapement & harvest ######

make.stikine.sk.plot<-function(stikine.sx){
  
  esc_harvest <- stikine.sx %>%
  ...
  labs(fill="",y="Number of Fish",x="Year",
       title="TBR: Stikine River",subtitle="Sockeye Salmon")
  
  cdn_us_harvest <- stikine.sx %>%
  ...
  percent_harvest <- stikine.sx %>%
  mutate(total.harvest = cdn_harvest+us_harvest) %>%
