...
tot.catch<-totsn%>%select(Area,Year,US.Catch,CDN.Catch,Total.Catch)%>%
  pivot_longer(3:5,names_to="Fishery",values_to="Catch")

ggplot(tot.catch,aes(x=Year,y=Catch/1000,color=Fishery))+
  geom_point(alpha=.5)+geom_line(alpha=.5)+
  theme_bw()+
  scale_colour_manual(values=c("red","black","blue"))+
  facet_wrap(~Area,ncol=1,scales="free_y")+
  labs(x="Year",y="Catch (Thousands)",
       title=paste0("US and CDN Total Catch of Skeena and Nass sockeye"))+
  theme(legend.position ="right")

# ggsave("SX SEAK vs CDN Catch.png",dpi=600,height=6,width=6)
